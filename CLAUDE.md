...
**Cron jobs** (in `main.ts`, via `node-cron`; `ENABLE_CRON` is read by the unused `cron.service.ts` and does not gate these): Netlink scraper daily at 23:00 (page = day of month). Monthly sequence 5 days before month end at 02:00: Paper.club then RocketLinks (spawned as child processes from `dist/cli/`), then DomDetailer in-process. Every job reports one record to the Dashboard's `POST /scraping/runs` via `ScrapingRunReporterService`.
